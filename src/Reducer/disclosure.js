...
export const emptyDisclosureAnswer = (data) => {
  switch (data.type) {
...
export const filterInvalidDisclosureAnswer = (data, answer) => {
  if (
    answer.value === '' ||
  ...
  return (state, action) => {
    ...
    switch (action.type) {
      case `@@${slice}/add_disclosure`:
        ...
        updatedAnswer = {};
        if (state.disclosures[action.id]) {
          updatedAnswer = state.disclosures[action.id].answer;
        ...
        return {
          ...
          disclosures: {
            ...
            [action.id]: {
              ...state.disclosures[action.id],
              question: {
                ...state.disclosures[action.id].question,
      ...
      case `@@${slice}/answer_current_disclosure`:
        currentQuestion = state.disclosures[action.questionId];
            ...
            case AURA_DISCLOSURE_TYPES.DATE:
              return {
                ...
                disclosures: {
                  ...state.disclosures,
                  [action.questionId]: {
                    ...state.disclosures[action.questionId],
                    answer: {
                      ...state.disclosures[action.questionId].answer,
                      value: {
                        ...state.disclosures[action.questionId].answer.value,
                        ...action.value,
                      },
                    },
                  },
...
const SLICE_NAME = 'disclosure';

export default (user) => (state = initialState, action) => (
  makeDisclosureHelper(`${user}/${SLICE_NAME}`)(state, action));
