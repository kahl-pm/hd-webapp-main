import React from 'react';

const Info = (props) => {
  return (
    <svg version="1.1" x="0px" y="0px" width="510px" height="510px" viewBox="0 0 510 510" style={{ enableBackground: 'new 0 0 510 510' }} shapeRendering="geometricPrecision">
      <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
        <g id="info-outline" fill={props.color} fillRule="nonzero">
          <path d="M229.5,382.5h51v-153h-51V382.5z M255,0C114.75,0,0,114.75,0,255s114.75,255,255,255s255-114.75,255-255S395.25,0,255,0z
             M255,459c-112.2,0-204-91.8-204-204S142.8,51,255,51s204,91.8,204,204S367.2,459,255,459z M229.5,178.5h51v-51h-51V178.5z"
          />
        </g>
      </g>
    </svg>
  );
};

export default Info;
